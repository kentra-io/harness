...
| **Arize Phoenix** ([docs](https://arize.com/docs/phoenix), [GitHub](https://github.com/Arize-ai/phoenix)) | Active, **Elastic License 2.0 (ELv2)**. Self-hostable (Docker/K8s); SQLite default, Postgres for production. | **Not selected — and a category note.** Flagged (user, 2026-06-29) as an "observability proxy/gateway alternative to LiteLLM," but it is **not** a gateway: Phoenix is built on **OpenTelemetry / OpenInference** and collects traces by **instrumentation + OTLP ingest**, not by proxying `/v1/messages`. It therefore can't replace LiteLLM (the routing/virtual-keys/budgets/passthrough layer) at all — it's a **Langfuse-class** observability+eval tool (tracing, LLM/code/human evals, prompt versioning, datasets/experiments). As a *Langfuse* alternative it's viable in principle (LiteLLM already exports OTel, so it could feed Phoenix instead), but **not chosen**, for two reasons: (1) **license** — ELv2 is a source-available, non-OSI license with use restrictions, exactly the relicensing/usage tail-risk §4.2 weighs against, whereas Langfuse is MIT; (2) Langfuse's datasets/experiments/eval-over-traces map more directly to the **eval-of-configurations** goal (§1). Keep on the bench as the obvious swap-in if Langfuse's MIT/self-host posture degrades (§4.2). |
