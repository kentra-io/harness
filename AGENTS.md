...
- [`kentra-skills/`](./kentra-skills/) — hand-authored architecture skills as a Claude Code plugin (public/MIT): `java-hexagonal` (framework-neutral JVM hexagonal; authoritative for kafka-dq per its ADR-0011) + `spring-boot-hexagonal`. Listed in the catalog. **Shipped 2026-07-19.** Headless box consumption pending (thread F).
- [`milestoned-plan-dag/`](./milestoned-plan-dag/) — framework-neutral, unbranded (MIT) DAG-of-milestones plan primitive: a machine-first YAML plan format describing a DAG of verifiable milestones, a Go CLI (`validate`/`resolve`/`render`), and an agent-agnostic authoring skill; no module-level constitution (deliberate design decision). **Shipped — pinned commit `feb0371`.**
...
- **Planned (Stage 3, not yet a repo) — spec drafted:** [`orchestration`](./orchestration.md) — the **execution** leg: the business logic that drives an approved `spec-lifecycle` plan to merged code through the agent cast. Unlike the other primitives it **extends** an engine (Microsoft Conductor) rather than owning one — Conductor is the durable spine (attempt counter, human-gate, resumability, crash-safety); the module adds the implement→verify→escalate loop, a **3-layer verification** (executable acceptance check + generic healthcheck + advisory judge; **author≠verifier** as the trust spine; diff-confined-to-declared-paths), and a fixed **3-attempt escalation ladder** (1 solo + 2 orchestrator-guided, human on the 3rd → `Needs human input`). Consumes `agent-definition` (the cast), `spec-lifecycle` (plan + gates + a small §5.5 additions change), claudebox (runtime + skills/plugins overlay). Neutral/branded cut + repo name still open (§14.1). Full spec: [`orchestration.md`](./orchestration.md); **implementation plan drafted 2026-07-07** (verified against live Conductor `main@7aaa589`/v0.1.20; fine-direct sequencing; module core = **Python**, engine-dictated): [`orchestration-implementation-plan.md`](./orchestration-implementation-plan.md).
