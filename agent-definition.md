...
- **Isolation.** Assignment alone is not isolation: preloaded-but-not-listed skills stay *discoverable* via the Skill tool, and a shared `~/.claude` puts the host operator's personal skills/plugins in every container. Three mechanisms were evaluated (research 2026-07-06); ranked by robustness and collateral:
  ...
  |---|---|---|---|
  | **Materialized `.claude` (`claude_dir_source`)** — the box's *whole* `~/.claude` is a caller-materialized artificial dir (skills/ + plugins/ + settings.json + role CLAUDE.md), copied in; only `.credentials.json` injected from host | strong (host operator scope *physically absent* — the whole `~/.claude` is purpose-built, not just two subdirs shadowed) | none (credentials injected separately; worktree skills intact) | **PRIMARY (default)** |
  ...
  **Decision (2026-07-06, revised 2026-07-07 → materialized `.claude`): the materialized `.claude` (`provisioning.claude_dir_source`) is the default**, made viable by the fact that **claudebox is our own vendored fork** (`claudebox/`) — the seam is a small change to code we own, not a fight with a third-party tool. The earlier plan (a *provisioning overlay* that shadow-mounted per-agent dirs over `~/.claude/skills` + `~/.claude/plugins` while still binding the host `~/.claude`) was superseded: shadowing only those two subdirs **still leaked the operator's global `CLAUDE.md`, `commands/`, `agents/`, and session/daemon state** into every box. Materializing a purpose-built `~/.claude` gives *true* isolation (host operator scope physically absent, not just two subdirs shadowed) and sidesteps an in-box host-home path-resolution bug (cb resolving the bind source via `os.UserHomeDir()` → the container path, which 403'd at the socket-proxy); the materialized dir lives under the change's worktree, so every mount is a project-subpath the socket-proxy accepts with no host-path resolution. `CLAUDE_CONFIG_DIR` (relocate the whole user scope) was rejected: undocumented and broken (open upstream bugs; not referenced in our claudebox source). `--bare` is retained only for the rare minimal-surface experiment.

**Two channels feed a headless agent** (and `claude_dir_source` only touches the first):
1. **Materialized `.claude` (per-agent):** the *whole* `~/.claude` is a purpose-built artificial dir (skills/ + plugins/ + settings.json + role CLAUDE.md), **copied** into the container (container-local, writable) — **no host `~/.claude` bind at all**; only `.credentials.json` is injected from the host. Nothing from the host operator's personal scope is present. This is the controlled channel. Materializing its *contents* is the orchestration/agent-definition module's job (`agentdef compile` seeds it, §5.2); claudebox just consumes a caller-materialized dir.
2. **Worktree (free, per-repo):** the target repo's `.claude/skills` — where `lifecycle/constitution init` install the primitives' *ongoing* stage/governance skills. Rides the existing worktree bind mount, already isolated per-repo, untouched by the materialized-`.claude` channel. So the primitives' skills reach an agent *for free* via the repo it works on; channel 1 provisions only cross-cutting skills (execution disciplines) + any bootstrap plugin a repo-initializing persona needs.

**The abstraction consequence (unchanged):** isolation is a **compile-and-launch policy owned by the runtime adapter, not a card field** — same discipline as `permissionMode` being a runtime constant (§3.2) and `tools` being launch-context-bound (§7). `x-skills` stays a pure, runtime-agnostic assignment list; the claudebox adapter maps it into the materialized `.claude`, and a second runtime's adapter maps it to *its* mechanism.

### 5.2 Provisioning & resolution — persona → skill/plugin sources

Seeding the materialized `.claude` means turning `x-skills` *names* into actual skill/plugin *files*. Sources are **loosely coupled** — skills/plugins live in their own repos (spec-lifecycle, adr-sourced-constitution, external Superpowers), indexed by the `kentra-agentic-plugins` catalog — **not** co-located with personas. The shape is a package manager:

- **Personas hold references** (`x-skills: [names]`), never bodies.
- **A resolution step** (`agentdef compile`) reads references → pulls each from its source (catalog / repo) → assembles the per-agent materialized `.claude` (skills/ + plugins/ + settings.json + role CLAUDE.md) that claudebox then copies in via `claude_dir_source` (§5.1). Optionally pinned by a **lockfile** for byte-reproducible variants (matters for Stage-5 A/B).
...
**Phasing (v1 = hardcoded, tooling later).** Prove the risky part first: v1 ships a **fixed source map / vendored copies** for the known cast + the claudebox `claude_dir_source` seam, and demonstrates end-to-end isolation. Deferred to a later stage: the general reference→source resolution manifest, lockfile, `agentdef` resolve/provision verbs, and skill-set **bundles** (named groups like `execution` so personas reference a bundle, not five skills). Same discipline spec-lifecycle shipped with — mechanism before ergonomics.
...
- **Per-variant skill injection = a per-variant materialized `.claude` (§5.1), not `--bare`.** When a variant overrides `x-skills`, compile assembles that variant's materialized `.claude` (its `skills/` subset) and claudebox copies it in — changing **only** the skill set while holding MCP, CLAUDE.md, governance, and model constant. This is *cleaner than `--bare` for A/B*: `--bare` would strip far more than skills, muddying the comparison unless every stripped thing is re-added identically. The materialized `.claude` isolates the one variable by construction. (`--bare` + explicit provisioning remains the fallback only for a rare "minimal-surface" experiment.) This corrects the earlier draft, which made variant isolation `--bare`-based before the §5.1 mechanism was chosen.
...
3. **claudebox `.claude` provisioning seam (the isolation mechanism, §5.1). ✅ BUILT 2026-07-07** (claudebox M2, PR #3, `kentra-io/claudebox` main @ `57a6ad6`). The shipped mechanism is **`provisioning.claude_dir_source`** — cb copies a caller-materialized artificial `~/.claude` into the container (container-local, writable) and injects only `.credentials.json` from the host; there is **no host `~/.claude` bind**. This *replaced* the originally-planned host-path shadow overlay (mounting per-agent dirs over `~/.claude/skills` + `~/.claude/plugins`), which still leaked the operator's global `CLAUDE.md`/`commands/`/`agents/`/session state. Consequently the host-path / `os.UserHomeDir()` concern in the original bullet is **moot** — with no host `~/.claude` bind there is no host path to resolve, and the materialized dir living under the change's worktree makes every mount a project-subpath the socket-proxy accepts. Live-DoD-proven: box `~/.claude/skills`+`plugins` show only materialized content, planted host operator decoys are absent, credentials injected but never written to the worktree source. Rejected alternative: `CLAUDE_CONFIG_DIR` (broken/undocumented, absent from our claudebox source).
