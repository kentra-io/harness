...
> **⏱ STATUS 2026-07-06 — SHIPPED + Option-B + Stage renumber (supersedes the banner below where they conflict).** **(1)** The planning module is **released**: [`spec-lifecycle`](./spec-lifecycle/spec-lifecycle.md) **v0.1.0, PR #1 → `main`** (M1–M8, tests green). **(2) P0 REVERSED to Option B (pure Go):** OpenSpec is the on-disk **format** we reimplement in Go — **no OpenSpec/Node runtime**. Every "OpenSpec (pinned) as artifact runtime" below is superseded — substitute "OpenSpec format, owned Go engine." **(3) Phase → Stage renumber:** this doc's **Phase 1** (planning vertical) = **Stage 2 (done)**; **Phase 2 (engine integration) + Phase 3 (execution) collapse into Stage 3** (orchestration + runtime + agent abstraction — [`tasks/orchestration-runtime-handoff.md`](./tasks/orchestration-runtime-handoff.md)); **Phase 4 (obs) = Stage 4**; auto-eval = **new Stage 5**. **Update 2026-07-24:** Stage 3 is now live/shipping — the `agent-orchestration` submodule is built and in production use, no longer merely planned.
