...
> **⏱ STATUS 2026-07-06 — SHIPPED + Option-B (supersedes the 2026-07-02 banner below on two points).** **(1) `spec-lifecycle` is RELEASED** (v0.1.0, PR #1 → `main`, cask in the tap) — M1–M8 built, `go test ./...` green — not "specced / pending review." **(2) P0 was REVERSED to Option B (pure Go):** `spec-lifecycle` conforms to the OpenSpec on-disk *format* but **reimplements the whole engine in Go — there is no OpenSpec/Node runtime**. Wherever the banner below (point 2) says "OpenSpec as the pinned artifact **runtime**," read "OpenSpec as a **format** we own a Go implementation of." **(3) Stage renumbering:** this doc's planning work = **Stage 2 (done)**; orchestration + runtime + agent-abstraction = **Stage 3** (planned — [`tasks/orchestration-runtime-handoff.md`](./tasks/orchestration-runtime-handoff.md)); obs = **Stage 4**; auto-eval = **Stage 5**. **Update 2026-07-24:** Stage 3 is now live/shipping — the `agent-orchestration` submodule is built and in production use, no longer merely planned.
