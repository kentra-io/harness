...
**DoD (live tier, fixture testbed):** a planted **undeclared deviation** and a planted **false completion** (`[x]` with no change) are each caught by the Verifier; a clean milestone passes all three layers; the L3 verdict is well-formed against the rubric; the Implementer halts (QUESTION) on a deliberately ambiguous task instead of improvising; Implementer has no WebSearch/WebFetch in-session.
